...
extension SwiftMethodParser {
     ...
     */
    var methodDef: Parser<BPMethodNode> {
        return curry(BPMethodNode.swiftInit)
            ~>* isStatic
            *<~ methodName
    ...
    var isStatic: Parser<Bool> {
        return (token(.cls) <|> token(.statical)) *> pure(true) <|> pure(false)
    }
    ...
    var methodName: Parser<String> {
        return token(.function) *> token(.name) ~>- go2String
    ...
    var param: Parser<Param> {
        let outter = token(.underline) *> pure("") // "_ param:"
            <|> lookAhead(token(.name) <* token(.colon)) ~>- go2String // "param:"
            <|> token(.name) ~>- go2String // "outter inner:"
        
        return curry(Param.swiftInit)
            ~>* outter
            *<~ token(.name) <* token(.colon) ~>- go2String
